...
export default function CollectionPage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          const pdfsResponse = await fetch(`/api/collections/${collectionId}/pdfs`, {
            headers: {
              Authorization: `Bearer ${token}`
            }
          })
  ...
  const confirmDeletePdf = async () => {
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(`/api/collections/${collectionId}/pdfs/${pdfToDelete}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`
        }
  ...
  const confirmModifyPdf = async () => {
    try {
      const token = localStorage.getItem("token")
      const response = await fetch(`/api/pdfs/${pdfToModify}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
        },
        body: JSON.stringify({ title: newPdfName })
      })
